...
public class ExactCoverProblem {
	...
	private void linkHoriz(Node left, Node right) {
		left.setRight(right);
		right.setLeft(left);
	}

	private void linkVert(Node top, Node bottom) {
		top.setDown(bottom);
		bottom.setUp(top);
	}
	...
	private void cover(Header header) {
		...
		while (colNode != header) {
			Node rowBase = colNode;
			Node rowNode = rowBase.getRight();
	...
	private void uncover(Header header) {
		Node colNode = header.getUp();
		while (colNode != header) {
			Node rowBase = colNode;
			Node rowNode = rowBase.getLeft();
			while (rowNode != rowBase) {
